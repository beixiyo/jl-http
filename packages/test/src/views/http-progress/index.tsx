    ...
    <div className="mx-auto max-w-6xl p-6">
        ...
        <Card className="mb-6 p-6">
          ...
          { downloadInfo && (
            <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
              <div>
                <div className="text-gray-600 font-medium dark:text-gray-400">已下载</div>
                <div className="text-lg font-semibold">
                  { formatBytes(downloadInfo.downloadedSize) }
                </div>
              </div>
              <div>
                <div className="text-gray-600 font-medium dark:text-gray-400">总大小</div>
                <div className="text-lg font-semibold">
                  { formatBytes(downloadInfo.totalSize) }
                </div>
              </div>
              <div>
                <div className="text-gray-600 font-medium dark:text-gray-400">下载速度</div>
                <div className="text-lg font-semibold">
                  { formatSpeed(downloadInfo.speed) }
                </div>
              </div>
              <div>
                <div className="text-gray-600 font-medium dark:text-gray-400">预计剩余</div>
                <div className="text-lg font-semibold">
                  { downloadInfo.estimatedTime > 0
                    ? formatTime(downloadInfo.estimatedTime)
                    : '--' }
                </div>
              </div>
            </div>
      ...
      { error && (
        <Card className="mb-6 border-red-200 bg-red-50 p-6 dark:border-red-800 dark:bg-red-900/20">
          <h2 className="mb-2 text-lg text-red-700 font-semibold dark:text-red-400">
            错误信息
          </h2>
      ...
      { result && (
        <Card className="mb-6 p-6">
          ...
          <div className="mb-2 text-sm text-gray-600 dark:text-gray-400">
            数据大小:
            {' '}
            { formatBytes(new Blob([result]).size) }
          </div>
          <div className="max-h-40 overflow-auto rounded bg-gray-100 p-3 text-xs font-mono dark:bg-gray-800">
            { result.length > 500
              ? (
                  <>
                    <div>前 250 个字符:</div>
      ...
      <Card className="p-6">
        ...
        <div className="max-h-60 overflow-auto rounded bg-gray-100 p-3 text-xs font-mono dark:bg-gray-800">
          { logs.length === 0
            ? (
